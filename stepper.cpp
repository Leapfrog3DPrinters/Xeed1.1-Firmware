...
//==========================================================================
//filament detectie door maarten logtenberg
...
FORCE_INLINE void checkFilament(){
  // start filament detectie door maarten logtenberg
  // Read new status
  bool newStatus = READ(DECTECTIE_PIN);
    
  // Check for a change in status
  if (newStatus != previousStatus){
    // status has changed, so reset counter
    stepCounter = 0;
    previousStatus = newStatus;
  }
  else{
    // Status not changed, so increase counter
    stepCounter++;
  }
    
  // Check whether counter is out of range (no status change within a certain amount of steps)
  if (stepCounter > FILAMENT_STEP_COUNTER_MAX){
    //delay(10000); 
    //Stop();
  }
}
...
//wb hack
#undef CHECK_DOORS
FORCE_INLINE void checkDeur(){
  #ifdef CHECK_DOORS
  bool deurStatus = READ(DETECTIE_DEUR_PIN);
  while(deurStatus == false){
    delay(10);
    deurStatus = READ(DETECTIE_DEUR_PIN);
  }
  #endif
}
