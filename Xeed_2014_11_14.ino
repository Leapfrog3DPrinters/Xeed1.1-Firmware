...
homing bed gaat alleen naaar beneden als hij binnen 15 mm van de nozzle staat.
ook bij de eerste keer opstarten zal hij dit doen aangezien hij dan denkt dat ie op 0 staat

bij het z-as homen kijkt hij of de kop minsten 20mm van de linker kant afstaat. als hij hierbinnen staat zal hij 20mm naar rechts bewegen.

hufterproof maken5:

de filament detectie gebeurt nu niks.. moet namelijk in de host software veranderen
...
unsigned long tempMillisPosDebug = 0;
bool showDebugPos = false;

void debugPosition(){
  if (showDebugPos){
    tempMillisPosDebug = millis();
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      // homing van de Xeed
      // eerst het bed 15mm omlaag
      // dan de X-as homen
      // vervolgens de kop 175mm naar rechts bewegen
      // dan de Y -as naar achteren homen
      // 175mm naar voren bewegen
      // de kop 2cm naar rechts bewegen (build in safety als alleen de z-as gehomed word)
      // de push solenoid uitklappen
      // z-as homen
      ...
      // z-as eerst 15mm omlaag

      if(current_position[Z_AXIS] <= 15){
        destination[Z_AXIS] = current_position[Z_AXIS]+16;
      ...
      if((home_all_axis) || (code_seen(axis_codes[X_AXIS])))
      {
        ...
        // kop 175mm naar rechts bewegen
        destination[X_AXIS] = 175;
        plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], 100, active_extruder);
        for(int8_t i=0; i < NUM_AXIS; i++) {
          current_position[i] = destination[i];
        }
        st_synchronize();
      ...
      if((home_all_axis) || (code_seen(axis_codes[Z_AXIS]))) {
        ...
        // kop eerst 2 cm naar rechts voor dat hij gaat homen.
        if(current_position[X_AXIS] <= 20){
          destination[X_AXIS] = current_position[X_AXIS]+21;
          plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], 100, active_extruder);
          for(int8_t i=0; i < NUM_AXIS; i++) {
            current_position[i] = destination[i];
          }
        ...
        // solenoid uitklappen en z-as homen
        WRITE(SOL2_PIN,HIGH);
        delay(800);
        HOMEAXIS(Z);
        WRITE(SOL2_PIN,LOW);
...
//      #endif
     ...
     // SERIAL_ECHOLN("Y Home");
      ...
      if(code_seen(axis_codes[Z_AXIS])) {
        if(code_value_long() != 0) {
          current_position[Z_AXIS]=code_value()+add_homeing[2]+Z_OFFSET;
        }
      }
    ...
    case 34: // G34 alleen 1ste z-as bewegen

      Z_STEPPER_SINGLE = 1;
      ////SERIAL_PROTOCOLPGM(" right front spindle active");
      break;

    case 35: // G35 alleen 2de z-as bewegen

      Z_STEPPER_SINGLE = 2;
      ////SERIAL_PROTOCOLPGM("back spindle active");
      break;

    case 36: // G36 alleen 3de z-as bewegen
    ...
    case 37: // G37 - inladen filament
      destination[E_AXIS] = current_position[E_AXIS] + 1050;
    ...
    case 38: // G38 - uitladen filament
          ...
          destination[E_AXIS] = current_position[E_AXIS] - 1200;
      plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], 3000/60, active_extruder);
       current_position[E_AXIS] = destination[E_AXIS];
         for(int8_t i=0; i < NUM_AXIS; i++) {
        current_position[i] = destination[i];
    ...
    case 40:{ // detectie sensor testen

      bool status1 = READ(DECTECTIE_PIN);
      SERIAL_PROTOCOL(getStepCounter());
      SERIAL_PROTOCOL(status1);
    }
      break;

    case 41:{ //deursensor testen

      bool status2 = READ(DETECTIE_DEUR_PIN);
      SERIAL_PROTOCOL(status2);
    }
      break;
      ...
      destination[i] = (float)code_value() + (axis_relative_modes[i] || relative_mode)*current_position[i];
       if(active_extruder == 1){
        if (i==0){
          ...
          destination[i] += EXTR2_Z_OFFSET;
        }
      }
      //else{
      //  if (i==2){
      //    destination[i] += Z_OFFSET;
      //  }
     // }
      seen[i]=true;
    }
...
void ZAdjust3(float Distance)
{
 ...
 float zprobe_3points_Willem(boolean CorrectionMove) {
      ...
      float correctiey_backmid = 455.00 * aY;
